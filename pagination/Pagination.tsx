...
export type Props = {
	...
	handleGoToPage: (
		...
		collection: PaginatedCollection<any> /*current collection*/
	) => any,
	/**
	 * Layout:
	 *    full: «« | «       » | »»
  ...
  intl = useIntl();
	...
	componentDidMount() {
		...
		if (this.props.layout === 'lazy') {
			this.scrollSource = this.props.scrollSource
				? document.querySelector(this.props.scrollSource)
				: window;
	...
	componentDidUpdate(prevProps: Props /*, prevState, prevContext*/) {
		...
		if (
			!!_.get(collection, 'page') &&
			_.get(collection, 'page') !== _.get(prevProps, 'collection.page')
		) {
	...
	renderFull() {
		return (
			<Sc.PaginationFull className="stackend-pagination stackend-pagination-full">
				...
				<button
					className="stackend-previous icon"
					onClick={this.onPreviousClicked}
					aria-label="Previous">
	...
	renderNumbered() {
		...
		return (
			<Sc.PaginationWrapper>
				...
				{collection.page !== collection.firstPage && (
					<Sc.PaginationButtons onClick={this.onFirstClicked}>
				...
				{collection.page + 1 < collection.lastPage && (
					<Sc.PaginationButtons
						onClick={() => this.goToPage(collection.page + 1)}>
						{collection.page + 1}
					</Sc.PaginationButtons>
				)}
				...
				{collection.page !== collection.lastPage && (
					<span style={{ display: 'flex' }}>
						...
						<Sc.PaginationButtons onClick={this.onLastClicked}>
							{collection.lastPage}
						</Sc.PaginationButtons>
					</span>
				)}
